...
class _EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('decode ', torch.min(output), torch.max(output), torch.isnan(output).any(), output.size())
...
class SegNet(nn.Module):
    ...
    def forward(self, x, feat=False):
        ...
        # print('enc1 ', torch.min(enc1), torch.max(enc1), torch.isnan(enc1).any(),
        #       torch.isnan(ind1).any(), enc1.size(), size1)
        (enc2, ind2), size2 = self.enc2(enc1)  # size [24, 128, 64, 64] # out [24, 128, 32, 32]
        # print('enc2 ', torch.min(enc2), torch.max(enc2), torch.isnan(enc2).any(),
        #       torch.isnan(ind2).any(), enc2.size(), size2)
        (enc3, ind3), size3 = self.enc3(enc2)  # size [24, 256, 32, 32] # out [24, 256, 16, 16]
        # print('enc3 ', torch.min(enc3), torch.max(enc3), torch.isnan(enc3).any(),
        #       torch.isnan(ind3).any(), enc3.size(), size3)
